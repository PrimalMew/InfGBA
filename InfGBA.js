/* An information bot created by PrimalMew.
Run with "node InfGBA.js --harmony". */
...
bot.on("ready", () => {
	...
	console.log(colors.cGreen("InfGBA is working!") + " Listening to " + bot.channels.length + " channels on " + bot.servers.length + " server(s).");
...
//	if (config.greet_new_members) { console.log("New member on " + objServer.name + ": " + objUser.username); bot.sendMessage(objServer.defaultChannel, "Welcome to the server, " + objUser.username.replace(/@/g, '') + "!"); }
//});
		...
		try {
			...
			bot.joinServer(msg.content, function(err, server) {
				if (err) {
					...
					var toSend = [];
					if (msg.author.id == '95687096504680448') { toSend.push("Greetings! I'm **InfGBA**!"); }
